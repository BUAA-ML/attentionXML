...
if [ ! -f data/$1/train_texts.npy ]; then
  python preprocess.py $TRAIN_TEXT --label-path ../datasets/$1/train_labels.txt #--vocab-path data/$1/vocab.npy --emb-path data/$1/emb_init.npy --w2v-model data/glove.840B.300d.gensim
fi
if [ ! -f data/$1/test_texts.npy ]; then
  python preprocess.py $TEST_TEXT --label-path ../datasets/$1/test_labels.txt #--vocab-path data/$1/vocab.npy
fi
